...
class PhoneVerificationServiceIntegrationTest {
  ...
  void confirmVerificationWithTestCodeMarksVerified() {
    ...
    var confirmResponse =
        phoneVerificationService.confirmVerification(requestResponse.verificationId(), "123456");

    assertThat(confirmResponse.signupVerificationToken()).isNotBlank();

    PhoneVerification verified =
        phoneVerificationRepository.findById(requestResponse.verificationId()).orElseThrow();
