...
```
1. 결제 완료 → SMS 로 /tickets?token= 링크 발송 (이미 동작)
2. 구매자가 모바일 브라우저로 링크를 열어 QR 화면을 본다 (이미 동작)
...
로컬 개발 중에는 서버 로그에서 인증번호를 확인해야 한다. 실연동은
...
`docs/screen-api-map.md` 4-2 절이 정리한 것들이다. 화면 자체는 만들되:
